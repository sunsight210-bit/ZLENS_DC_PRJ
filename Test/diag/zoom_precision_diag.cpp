...
// 测试点列表: 从小到大顺序走, 不回零
constexpr int32_t OFFSETS[] = {-100, -50, -20, -10, -5, -2, 0, +2, +5, +10, +20, +50, +100};
...
static void diag_task_entry(void* params) {
    ...
    swo_printf("[DIAG] === 3.0X OFFSET PRECISION TEST (NO RE-HOME) ===\n");
    swo_printf("[DIAG] 3.0X pos=%ld, sequential: -100..+100\n",
               static_cast<long>(POS_3X));
    uart_puts("\r\n=== 3.0X OFFSET PRECISION TEST (NO RE-HOME) ===\r\n");
    uart_puts("Homing once, then sequential moves. Send any key to advance.\r\n\r\n");
    ...
    for (uint32_t i = 0; i < NUM_OFFSETS; ++i) {
        ...
        snprintf(aBuf, sizeof(aBuf),
                 "\r\n[%lu/%lu] Moving to 3.0X %+ld counts (target=%ld)...\r\n",
